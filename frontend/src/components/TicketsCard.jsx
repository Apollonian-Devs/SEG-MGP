              ...
              <GenericTable
                columnDefinition={[
                  <th className="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase cursor-pointer">
                    <GenericButton className="flex items-center w-full gap-x-1" onClick={() => sortTickets("subject")}>
                      <p>Subject</p>
                      {sortConfig.key === "subject" ? (sortConfig.direction === "asc" ? "▲" : "▼") : ""}
                    </GenericButton>
                  </th>,
                  <th className="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase cursor-pointer">
                    <GenericButton className="flex items-center w-full gap-x-1" onClick={() => sortTickets("status")}>
                      <p>Status</p>
                      {sortConfig.key === "status" ? (sortConfig.direction === "asc" ? "▲" : "▼") : ""}
                    </GenericButton>
                  </th>,
                  <th className="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase cursor-pointer">
                    <GenericButton className="flex items-center w-full gap-x-1" onClick={() => sortTickets("priority")}>
                      <p>Priority</p>
                  ...
                  user.is_staff && <th className="px-6 py-3 text-end text-xs font-medium text-gray-500 uppercase">Redirect</th>,
                ].filter(Boolean)}
                data={tickets}
                dataName="tickets"
                rowDefinition={(ticket) => [
                  <td key="subject" className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">
                    {ticket.subject}
                  </td>,
                  <td key="status" className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{ticket.status}</td>,
                  <td key="priority" className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                    {ticket.priority || "Not Set"}
                  </td>,
                  <td key="actions" className="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                    <GenericButton
                  ...
                  user.is_staff && (
                    <td key="redirect" className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
